...
describe('sharedLinks', () => {
  ...
  scenario('creates a sharedLink', async () => {
    const result = await createSharedLink({
      input: {
        updatedAt: '2024-11-21T22:47:33.684Z',
        title: 'String',
        url: 'String',
      },
    })

    expect(result.updatedAt).toEqual(new Date('2024-11-21T22:47:33.684Z'))
  ...
  scenario('updates a sharedLink', async (scenario: StandardScenario) => {
    ...
    const result = await updateSharedLink({
      id: original.id,
      input: { updatedAt: '2024-11-22T22:47:33.684Z' },
    })

    expect(result.updatedAt).toEqual(new Date('2024-11-22T22:47:33.684Z'))
  })
